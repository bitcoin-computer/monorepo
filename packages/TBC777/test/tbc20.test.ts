...
import { TBC20, TBC20Helper } from '../src/tbc20.js'
...
describe.skip('TBC20', async () => {
  let token1: SmartContract<typeof TBC20>

  describe('mint', async () => {
    it('Sender mints 3 tokens', async () => {
      const params = { to: sender.getPublicKey(), amount: 3n, name: 'test' }
      token1 = await sender.new(TBC20, [params])
    })
  ...
  describe('transfer, merge and burn', () => {
    let token2: SmartContract<typeof TBC20>
    let token2After: SmartContract<typeof TBC20>
    ...
    it('computer.getOUTXOs should return the tokens', async () => {
      ...
      const senderToken = await sender.sync<typeof TBC20>(senderRevs[0])
      ...
      const receiverTokens = await receiver.sync<typeof TBC20>(receiverRevs[0])
    ...
    it('Receiver send token2 back to sender', async () => {
      ...
      const { env } = effect
      const { __bc__ } = env
      token2After = __bc__ as SmartContract<typeof TBC20>
    ...
    it('Should update the revisions correctly', async () => {
      ...
      const params = { to: computer.getPublicKey(), amount: 3n, name: 'test' }
      const t1 = await computer.new(TBC20, [params])
      const rev1 = t1._rev
...
describe('TokenHelper', () => {
  ...
  describe('transfer', () => {
    ...
    it('Should fail if the amount is greater than the balance', async () => {
      ...
      it('Should fail if the amount is greater than the balance', async () => {
        const computer3 = new Computer({ url, chain, network })
        const tbc20 = new TBC20Helper(sender)
