...
const { CHAIN: chain = conf.DEFAULT_CHAIN, NETWORK: network = conf.DEFAULT_NETWORK, URL: url = conf.DEFAULT_URL, MNEMONIC: mnemonic, } = process.env;
if (network !== 'regtest') {
    if (!mnemonic)
...
if (answer === 'n') {
    ...
    `);
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].startsWith('POW'))
                lines[i] = `POW=${powModSpec}`;
        }
