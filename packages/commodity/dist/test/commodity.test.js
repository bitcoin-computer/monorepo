...
async function ensureFunds(c, minSats = 2e8) {
    try {
 ...
 */
async function fundedComputer(utxos = 2, sats = 5e8) {
    const computer = new Computer({ url, chain, network });
    for (let i = 0; i < utxos; i++)
        await computer.faucet(sats);
    return computer;
}
/**
 ...
 */
async function mineBlocks(computer, n = 1) {
    const hashes = await computer.rpc('generateToAddress', `${n} ${MINE_SINK_ADDRESS}`);
 ...
 */
async function confirmMint(computer, mint, modSpec) {
...
async function waitForMintInBlockIndex(computer, mod, mintId, blockHeight, timeoutMs = 30_000) {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
        // Match claim(): all object TXOs in the block (spent or unspent).
        const revs = await computer.getOTXOs({ mod, blockHeight });
        if (revs.includes(mintId))
            return;
        await sleep(500);
    }
    throw new Error(`Timed out waiting for ${mintId} in getOTXOs({ mod, blockHeight: ${blockHeight} })`);
}
/**
 ...
 */
async function deployCommodity(computer) {
 ...
 */
async function createMint(computer, modSpec, salt = `salt-${Math.random().toString(36).slice(2)}`) {
...
async function twoMintsSameBlock(saltA, saltB) {
    ...
    if (heightA !== heightB) {
        throw new Error(`Expected same-block mints, got heights ${heightA} and ${heightB}. ` +
...
async function expectClaimFails(mint, msg) {
    let thrown;
    try {
        await mint.claim();
    }
...
describe('Commodity – Canonical Min-Revision Digital Commodity', function () {
    ...
    describe('Constructor and minting', () => {
        describe('Genuine mint path (salt non-empty, amount must be 0n)', () => {
            it('creates a mint root with amount 0n, non-empty salt, and _id === _rev === _root', async () => {
                const salt = 'salt-abc';
                const mint = await alice.new(Commodity, [{ to: alice.getPublicKey(), salt, amount: 0n }], mod);
            ...
            it('throws when a mined Commodity is constructed with amount !== 0n', () => {
                ...
                expect(() => new Commodity({ to: pk, salt: 'salt', amount: 1n })).to.throw('Mined Commodity must start with amount === 0n');
            });
        ...
        describe("Transfer / split construction path (salt === '')", () => {
            ...
            it('throws if amount is negative', () => {
                expect(() => new Commodity({ to: bob.getPublicKey(), salt: '', amount: -1n })).to.throw('Amount cannot be negative');
            });
    ...
    describe('isGenuine() and lineage via immutable _root', () => {
        ...
        it('returns true for any transfer/split descendant that inherits the same _root', async () => {
            ...
            const child = (await mint.transfer(bob.getPublicKey(), half));
        ...
        it('returns true even for deep descendants (only the short root is ever synced)', async () => {
            ...
            const child = (await mint.transfer(bob.getPublicKey(), quarter));
            ...
            const grand = (await childAsBob.transfer(alice.getPublicKey(), quarter / 2n));
        ...
        it('returns false when the root itself was created with empty salt (fake / non-mint lineage)', async () => {
            const fake = await alice.new(Commodity, [{ to: alice.getPublicKey(), salt: '', amount: 10n }], mod);
    ...
    describe('transfer()', () => {
        ...
        describe('Partial transfer / split (amount argument)', () => {
            it('deducts amount from the parent and returns a new Commodity owned by the recipient', async () => {
                ...
                const send = total / 3n;
                const child = (await mint.transfer(bob.getPublicKey(), send));
            ...
            it('the child inherits the exact same _root (lineage preserved)', async () => {
                const { mint } = await mintClaimAndGet();
                const child = (await mint.transfer(bob.getPublicKey(), 1n));
                expect(child._root).to.eq(mint._root);
            });
    ...
    describe('claim() – Tier 1: eligibility guards (no same-block control required)', () => {
        it('throws if claim() is called before the mint creation tx is confirmed', async () => {
            ...
            await expectClaimFails(mint, /Accessing non-existent on-chain state inside a smart contract is forbidden/);
        });
        ...
        it('throws if the lineage is not genuine (isGenuine() === false)', async () => {
            ...
            const localMod = await deployCommodity(local);
            await mineBlocks(local, 1);
            const fake = await local.new(Commodity, [{ to: local.getPublicKey(), salt: '', amount: 0n }], localMod);
    ...
    describe('claim() – Tier 2: canonical selection (same-block control required)', () => {
        ...
        it('includes transfer/split children tagged with the same mod when selecting the min revision', async () => {
            ...
            const child = await local.sync(childRev);
        ...
        it('is history-independent and deterministic: two Computer instances agree on claim outcome', async () => {
            ...
            const synced = await other.sync(mint._rev);
    ...
    describe('Mining / claiming workflow (end-to-end happy path)', () => {
        it('full happy path: grind-ish salt → mint → confirm → claim → transferable', async () => {
            ...
            const mint = await local.new(Commodity, [{ to: local.getPublicKey(), salt, amount: 0n }], localMod);
            expect(mint.amount).to.eq(0n);
            ...
            const child = (await mint.transfer(recipient.getPublicKey(), subsidy / 2n));
    ...
    describe('Invariants', () => {
        ...
        it('transfer children remain fully functional tokens of the same lineage even though they can never claim', async () => {
            const { mint, computer } = await mintClaimAndGet();
            const child = (await mint.transfer(bob.getPublicKey(), 2n));
            expect(await child.isGenuine()).to.eq(true);
            // Only bob can spend the child UTXO.
            await mineBlocks(computer, 1);
            await ensureFunds(bob, 2e8);
            const childAsBob = await asOwner(bob, child._rev);
            const grandchild = (await childAsBob.transfer(alice.getPublicKey(), 1n));
            ...
            await expectClaimFails(childLatest, /claim\(\) can only be called on the mint creation revision/);
            await expectClaimFails(grandchild, /claim\(\) can only be called on the mint creation revision/);
        });
    });
});
// ---------------------------------------------------------------------------
...
describe('Sha256', () => {
    it('matches the empty-string SHA-256 digest', () => {
        expect(Sha256.hash('')).to.eq('e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855');
    });
    it('matches the well-known "abc" SHA-256 digest', () => {
        expect(Sha256.hash('abc')).to.eq('ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
    });
