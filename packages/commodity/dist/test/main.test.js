...
describe('Sha256 (pure JS implementation matching OpenSSL sha256.c)', () => {
    it('should match official SHA-256 test vectors (from NIST / OpenSSL test suite)', () => {
        ...
        expect(Sha256.hash('')).to.equal('e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855');
        // "abc" (NIST FIPS 180-2)
        expect(Sha256.hash('abc')).to.equal('ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
        // "hello" (standard example used in many OpenSSL tests)
        expect(Sha256.hash('hello')).to.equal('2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824');
        // Longer message (NIST / OpenSSL "quick brown fox")
        expect(Sha256.hash('The quick brown fox jumps over the lazy dog')).to.equal('d7a8fbb307d7809469ca9abcb0082e4f8d5651e46d3cdb762d02d0bf37c9e592');
    });
...
describe('Pow', () => {
    ...
    it('should mint the genesis token (prevMintedId = "", difficulty = 16)', async () => {
        ...
        const myToken = await computer.new(Pow, [computer.getPublicKey(), 1n, nonce.toString(), prevMintedId, difficulty], mod);
    ...
    it('should reject invalid proof-of-work (wrong nonce)', async () => {
        try {
            await computer.new(Pow, [computer.getPublicKey(), 1n, 'this-is-not-a-valid-nonce', '', 16], mod);
    ...
    it('should create a dummy object during transfer (nonce = "")', async () => {
        ...
        const token = await computer.new(Pow, [computer.getPublicKey(), 1n, nonce.toString(), prevMintedId, difficulty], mod);
        ...
        const split = (await token.transfer(computer.getPublicKey(), 1n));
    ...
    it('should allow merge of tokens', async () => {
        ...
        const makeToken = async () => {
            ...
            while (true) {
                ...
                if (crypto.createHash('sha256').update(puzzle).digest('hex').startsWith('0000'))
                    break;
                n++;
            }
    ...
    it('should allow mint with difficulty 0 (genesis or test)', async () => {
        const freshMod = await computer.deploy(`export ${Pow}; export ${Sha256};`);
        const token = await computer.new(Pow, [computer.getPublicKey(), 1n, 'any-nonce', '', 0], freshMod);
