...
export class Pow extends Contract {
    constructor(to, amount, nonce, prevMintedId, difficulty) {
    ...
    static isValidPow(nonce, prevMintedId, difficulty) {
        if (!nonce)
            return true;
        const puzzle = prevMintedId + nonce + difficulty.toString();
        const hash = Sha256.hash(puzzle);
        const targetZeros = Math.floor(difficulty / 4);
