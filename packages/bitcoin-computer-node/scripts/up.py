...
if(args.bitcoin):
    ...
    if(args.testnet):
        subprocess.run(
            ['sh', '-c', 'source scripts/aws-config.sh && aws_secret_export && docker compose -f docker-compose.yml -f chain-setup/ltc-testnet/docker-compose-local-ltc-testnet.yml up'])
    else:
        subprocess.run(
