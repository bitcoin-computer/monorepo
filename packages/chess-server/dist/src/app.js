...
app.get('/hash', async (_, res) => {
    try {
        ...
        await db.none(`INSERT INTO "Secrets" ("secret", "hash") VALUES ($1, $2);`, [secret, hash]);
...
app.get('/secret/:id', async (req, res) => {
    try {
        ...
        const contract = await computer.sync(rev);
        ...
        if (game.isGameOver()) {
            ...
            const { secret } = await db.one(`SELECT secret FROM "Secrets" WHERE "hash"=$1;`, [hash]);
