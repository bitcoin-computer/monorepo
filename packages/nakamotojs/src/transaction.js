...
export class Transaction {
    ...
    updateInput(inputIndex, opts) {
        ...
        if (typeof hash !== 'undefined')
            this.ins[inputIndex].hash = hash;
        if (typeof txId !== 'undefined')
            this.ins[inputIndex].hash = bufferUtils.reverseBuffer(Buffer.from(txId, 'hex'));
        if (typeof index !== 'undefined')
            this.ins[inputIndex].index = index;
    ...
    updateOutput(outputIndex, opts) {
        ...
        if (typeof value !== 'undefined')
            this.outs[outputIndex].value = value;
    }
    hasWitnesses() {
    ...
    sign(inIndex, keyPair, sighashType, prevOutScript) {
        ...
        const { finalScriptSig, finalScriptWitness } = prepareFinalScripts(prevOutScript, scriptType, partialSig, isSegwit, isP2SH, isP2WSH);
    ...
    hashForWitnessV1(inIndex, prevOutScripts, values, hashType, leafHash, annex) {
        ...
        typeforce(types.tuple(types.UInt32, typeforce.arrayOf(types.Buffer), typeforce.arrayOf(types.SmallSatoshi), types.UInt32), arguments);
    ...
    hashForWitnessV0(inIndex, prevOutScript, value, hashType) {
        typeforce(types.tuple(types.UInt32, types.Buffer, types.SmallSatoshi, types.UInt32), arguments);
    ...
    getInRevs() {
        return this.ins.map((i) => `${bufferUtils.reverseBuffer(i.hash).toString('hex')}:${i.index}`);
    }
}
