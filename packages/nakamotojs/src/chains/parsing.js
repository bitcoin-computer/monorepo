...
const alwaysParse = () => true;
const alwaysIndex = () => true;
 ...
 */
export function isMwebTxHex(hex) {
    if (hex.slice(ADVANCED_TX_MARKER_OFFSET, ADVANCED_TX_FLAG_OFFSET) !==
        ADVANCED_TX_MARKER) {
        return false;
    }
 ...
 */
export function skipTxsAtHeight(skips) {
...
export function getParsingPolicy(chain, network) {
...
export function shouldParseTransaction(hex, chain, network) {
    return getParsingPolicy(chain, network).shouldParse(hex);
}
/** Test helper: resolves policy then calls `shouldIndex`. */
export function shouldIndexTransaction({ txId, chain, network, height, }) {
