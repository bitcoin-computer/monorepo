...
import { p2pkh, p2sh, p2wpkh, p2wsh, p2tr, p2ms, p2pk, } from './payments/index.js';
import { tapleafHash } from './payments/bip341.js';
import * as bscript from './script.js';
import { Transaction } from './transaction.js';
import { toXOnly, tapScriptFinalizer, serializeTaprootSignature, isTaprootInput, checkTaprootInputFields, checkTaprootOutputFields, tweakInternalPubKey, checkTaprootInputForSigs, } from './psbt/bip371.js';
import { witnessStackToScriptWitness, checkInputForSig, pubkeyInScript, isP2MS, isP2PK, isP2PKH, isP2WPKH, isP2WSHScript, isP2SHScript, } from './psbt/psbtutils.js';
...
export class Psbt {
    ...
    get inputsAmount() {
        const inputsAmounts = this.data.inputs.map((input, index) => {
            if (input.witnessUtxo)
                ...
                const bigValue = Transaction.fromBuffer(input.nonWitnessUtxo).outs[txin.index].value;
    ...
    extractTransaction(disableFeeCheck) {
        ...
        inputFinalizeGetAmts(this.data.inputs, tx, c, true, disableFeeCheck || false);
        return tx;
    }
    getFeeRate() {
...
function getTaprootHashesForSig(inputIndex, input, inputs, pubkey, cache, tapLeafHashToSign, allowedSighashTypes) {
    ...
    const tapLeafHashes = (input.tapLeafScript || [])
        ...
        .map(tapLeaf => {
        const tapScriptHash = unsignedTx.hashForWitnessV1(inputIndex, signingScripts, values, Transaction.SIGHASH_DEFAULT, tapLeaf.hash);
        return {
            pubkey,
            hash: tapScriptHash,
...
function inputFinalizeGetAmts(inputs, tx, cache, mustFinalize, disableFeeCheck) {
    let inputAmount = 0;
    inputs.forEach((input, idx) => {
        ...
        if (input.witnessUtxo) {
            ...
            if (out.value > Number.MAX_SAFE_INTEGER)
                throw new Error('Cannot finalize input with value greater than safe integer');
