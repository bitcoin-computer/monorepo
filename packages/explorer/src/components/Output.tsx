...
function Output(props: { computer: Computer }) {
  ...
  const isBasic = (el: unknown) => ['undefined', 'boolean', 'number', 'string'].includes(typeof el)
  ...
  const format = (value: any) => {
    ...
    if(isObject(value))
      return revLink(value._rev)

    if(isTable(value))
      return (<table className="w-full mt-2 mb-8 text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          {value.map((row: any) => {
            return (
              <tr key={row._rev} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
