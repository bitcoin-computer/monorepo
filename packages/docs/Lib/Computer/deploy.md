...
- **`taproot`** — Two transactions (commit + reveal). The commit tx locks a taproot output whose leaf commits to the data; the reveal tx spends that output and embeds the full module source in the **input witness**, inside a script-path envelope tagged with protocol id **`BC`**. Content type is `text/javascript`. These modules can be close to 400 kB or even close to 4 MB (if you know a miner that will include them). Module storage is 4× cheaper due to the SegWit discount. _Crucially, taproot module encoding transactions generate no hygiene dust outputs._

Encryption of module payloads is not supported yet (may be added later). Only new-format deploys (`{ ept }` / `BC`) are valid; legacy shapes that stored modules as transition `exp` fields or non-`BC` inscription envelopes are not read.
