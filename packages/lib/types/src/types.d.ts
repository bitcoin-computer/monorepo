...
type Compute<T> = T extends (...args: any[]) => any
  ? T
  : {
      [K in keyof T]: T[K]
    } & object
...
export type DeepExact<T> = T extends
  | Contract
  | {
      readonly [brand]: any
    }
    ...
    : T extends readonly any[]
      ...
      : T extends any[]
        ...
        : T extends object
          ...
          : T
export type Lifted<T, Root extends Class = Class> =
  IsContractClass<T> extends true
    ? T extends InstanceType<infer C extends Class>
      ? SmartContract<C>
      : SmartContract<Root>
    : T extends Promise<infer U>
      ? Promise<Lifted<U, Root>>
      : T extends
            | Contract
            | {
                readonly [BC_BRAND]: true
        ...
        ? SmartContract<Root>
        : T extends (this: any, ...args: infer P extends readonly any[]) => infer R
          ? LiftedThisMethod<P, R, Root>
          : T extends (...args: infer P extends readonly any[]) => infer R
            ? LiftedFunction<P, R, Root>
            : T extends any[]
              ? {
...
export type LiftedReturn<R, Root extends Class = Class> =
  R extends Promise<infer U> ? Promise<Lifted<U, Root>> : Promise<Lifted<R, Root>>
export type LiftedThisMethod<Args extends readonly any[], Ret, Root extends Class> = (
  this: SmartContract<Root>,
  ...args: LiftedParameters<Args, Root>
) => LiftedReturn<Ret, Root>
export type LiftedFunction<Args extends readonly any[], Ret, Root extends Class = Class> = (
  ...args: LiftedParameters<Args, Root>
) => LiftedReturn<Ret, Root>
...
export type SmartContract<T extends Class = Class> = Compute<
  Exact<LiftedInstanceProperties<T> & Contract>
> & {
  readonly constructor: LiftedContract<T>
}
export type SmartContractBranded<T extends Class = Class> = Compute<
  Exact<LiftedInstanceProperties<T> & ContractBranded>
> & {
  readonly constructor: LiftedContract<T>
}
...
export type LiftedContract<T extends Class = Class> = Compute<SmartContract<T> & LiftedStatics<T>> &
  (new (...args: LiftedConstructorParams<T>) => SmartContract<T>) &
  (new (...args: any[]) => SmartContract<T>)
...
export declare function precise<T extends Class>(
  value: SmartContract<any>,
  opts?: {
    branded?: false
  },
...
export type RawSmartContract<T extends Class = Class> = Omit<
  SmartContract<T>,
  '_id' | '_rev' | '_root'
> & {
  _id: Id
...
export type OnChainMetadata = JsonObject &
  Partial<
    OwnerData & {
      ioMap: number[]
    }
...
export type ComputeOptions = VerifyOptions &
  FundOptions &
  AncestorOptions &
  SigOptions &
  MockOptions &
