...
describe('getTXOs', () => {
  ...
  describe('Get by gteBlockHeight', () => {
    it('Should get TXOs by gteBlockHeight', async () => {
      ...
      await sleep(2000)
      const blockInfo = await computer.rpc('getBlock', `${blockHex[0]} 1`)
      const txos = await computer.getTXOs({ gteBlockHeight: blockInfo.height })
      expect(txos.length).to.be.greaterThan(0)
      expect(txos).to.include(c2._rev)
    })
  })
  describe('Get by lteBlockHeight', () => {
    it('Should get TXOs by lteBlockHeight', async () => {
      ...
      await sleep(2000)
      const blockInfo = await computer.rpc('getBlock', `${blockHex[0]} 1`)
      const txos = await computer.getTXOs({ lteBlockHeight: blockInfo.height })
      expect(txos.length).to.be.greaterThan(0)
      expect(txos).to.include(c2._rev)
    })
  })
  describe('Get TXOs by blockIndex', () => {
    it('Should get TXOs by blockIndex', async () => {
      ...
      const txosIdx0 = await computer.getTXOs({ blockIndex: 0, blockHash: blockInfo.hash })
      expect(txosIdx0.length).to.be.greaterThan(0)
      expect(txosIdx0).to.include(`${blockInfo.tx[0]}:0`)
      const txosIdx1 = await computer.getTXOs({ blockIndex: 1, blockHash: blockInfo.hash })
      expect(txosIdx1.length).to.be.greaterThan(0)
      expect(txosIdx1).to.include(`${blockInfo.tx[1]}:0`)
    })
  })
