...
const randomPublicKey =
  "023e21361b53bb2e625cc1f41d18b35ae882e88d8d107df1c3711fa8bc54db8fed";
const randomRev =
...
describe("Fungible Token", () => {
  ...
  describe("Transferring a fungible token", async () => {
    ...
    it("If Sender mints another token it will have a different root", async () => {
      const fakeToken = await sender.new(Token, [
        sender.getPublicKey(),
        10,
        10,
      ]);
    ...
    it("Eve cannot read the content of the chat", async () => {
      try {
        ...
        expect(err.message).eq(
          "Cannot read properties of null (reading 'exp')"
        );
      }
    });
  });
