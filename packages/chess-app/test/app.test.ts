...
describe('ChessContractHelper', () => {
  ...
  describe('spend', () => {
    ...
    it('Should throw an error when trying to spend with the wrong secret', async () => {
      const tx = await chessContractHelperW.makeTx()
      const txId = await chessContractHelperB.completeTx(tx)
      await expect(chessContractHelperW.spendWithSecret(txId, secretB, 0)).rejects.toThrow('mandatory-script-verify-flag-failed (Script evaluated without error but finished with a false/empty top stack element)')
      await expect(chessContractHelperW.spendWithSecret(txId, secretB, 1)).rejects.toThrow('mandatory-script-verify-flag-failed (Signature must be zero for failed CHECK(MULTI)SIG operation)')
    }, 20000)
  })
})
