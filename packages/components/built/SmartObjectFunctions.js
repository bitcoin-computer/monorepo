...
function sourceOf(smartObject, name) {
    try {
        const fn = getMethodFn(smartObject, name);
        if (!fn)
            return '';
...
function CopyCode({ text }) {
    ...
    return (_jsx("button", { type: "button", onClick: () => {
        ...
        }, className: "text-[11px] font-medium text-blue-600 dark:text-blue-400 hover:underline shrink-0", children: copied ? 'Copied' : 'Copy' }));
}
export const SmartObjectFunctions = ({ smartObject, functionsExist, options, latestRev, compact = false, }) => {
    ...
    return (_jsxs("section", { className: "rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm overflow-hidden", "aria-label": "Methods", children: [_jsxs("div", { className: "px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-2", children: [_jsxs("div", { children: [_jsx("h2", { className: "text-base sm:text-lg font-semibold text-gray-900 dark:text-white", children: "Methods" }), _jsxs("p", { className: "text-xs text-gray-500 dark:text-gray-400 mt-0.5", children: [methods.length, " method", methods.length === 1 ? '' : 's', compact ? '' : ' · select one to call'] })] }), _jsx("span", { className: "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-blue-50 text-blue-800 dark:bg-blue-950/50 dark:text-blue-300 tabular-nums", children: methods.length })] }), isHistorical ? (_jsxs("div", { className: "px-4 py-2.5 border-b border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/30 text-sm text-amber-900 dark:text-amber-200", children: ["You are viewing a historical revision. Method calls that spend this object will fail.", ' ', _jsx(Link, { to: `/objects/${latestRev}`, className: "font-medium underline underline-offset-2 hover:opacity-90", children: "Go to latest revision \u2192" })] })) : null, compact ? (_jsx("div", { className: "p-4 sm:p-5 max-w-2xl", children: methods.map((name) => (_jsx(SmartObjectFunction, { funcName: name, smartObject: smartObject, functionsExist: true, options: options, latestRev: latestRev }, name))) })) : (_jsxs("div", { className: "flex flex-row items-stretch min-h-[14rem] overflow-x-auto", children: [_jsx("nav", { className: "shrink-0 w-44 sm:w-52 border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60", "aria-label": "Method list", children: _jsx("ul", { className: "p-1.5 space-y-0.5 overflow-y-auto max-h-80", role: "listbox", "aria-label": "Available methods", children: methods.map((name) => {
                            ...
                            }) }) }), _jsxs("div", { className: "flex-1 min-w-[12rem] flex flex-col border-r border-gray-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-950/40", "aria-label": "Method source", children: [_jsxs("div", { className: "px-3 py-2 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between gap-2", children: [_jsx("p", { className: "text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400", children: "JavaScript" }), _jsx(CopyCode, { text: selectedSource })] }), selectedSource ? (_jsx("pre", { className: "p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-auto max-h-80 whitespace-pre-wrap break-words leading-relaxed", children: highlightedSource })) : (_jsx("p", { className: "p-3 text-sm text-gray-500 dark:text-gray-400", children: "Source unavailable for this method." }))] }), _jsx(SmartObjectFunction, { funcName: activeMethod, smartObject: smartObject, functionsExist: true, options: options, latestRev: latestRev, embedded: true }, activeMethod)] }))] }));
};
