...
function publicEntries(value) {
    if (value == null || typeof value !== 'object')
        return [];
    return Object.entries(value).filter(([key, v]) => !keywords.includes(key) && typeof v !== 'function');
}
...
function NestedStateTable({ value, depth }) {
    ...
    const entries = Array.isArray(value)
        ...
        : publicEntries(value);
    return (_jsxs("div", { className: "mx-3 sm:mx-4 mb-2 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50/70 dark:bg-gray-800/40 overflow-hidden", children: [nestedRev ? (_jsx("div", { className: "px-3 py-1.5 border-b border-gray-100 dark:border-gray-700", children: _jsxs(Link, { to: `/objects/${nestedRev}`, className: "text-[11px] font-medium text-blue-600 dark:text-blue-400 hover:underline font-mono break-all", children: ["View object ", truncateRev(nestedRev), " \u2192"] }) })) : null, entries.length === 0 ? (_jsx("p", { className: "px-3 py-2 text-xs text-gray-500 dark:text-gray-400", children: "No public properties." })) : (entries.map(([key, nested]) => (_jsx(StateValueRow, { name: key, value: nested, nested: true, depth: depth + 1 }, key))))] }));
}
function StateValueRow({ name, value, nested = false, depth = 0, }) {
    ...
    const expandedString = isLongString
        ...
        : null;
    return (_jsxs("div", { className: "border-b border-gray-100 dark:border-gray-800 last:border-0", children: [_jsxs("div", { className: `${nested ? 'px-3' : 'px-3 sm:px-4'} py-1.5 flex items-center gap-2 sm:gap-3 min-h-0`, children: [_jsxs("div", { className: "shrink-0 flex items-center gap-1.5 w-[7.5rem] sm:w-36", children: [_jsx("span", { className: "text-xs font-medium text-gray-800 dark:text-gray-200 whitespace-nowrap truncate", children: capitalizeFirstLetter(name) }), _jsx("code", { className: "text-[10px] text-gray-400 dark:text-gray-500 whitespace-nowrap hidden sm:inline", children: name })] }), _jsx(TypeBadge, { type: type }), _jsx("div", { id: !nested ? `property-${name}-value` : undefined, className: "min-w-0 flex-1 text-xs text-gray-700 dark:text-gray-300 truncate", children: preview }), _jsxs("div", { className: "flex items-center gap-0.5 shrink-0", children: [full && full !== 'null' && full !== 'undefined' ? _jsx(Copy, { text: full }) : null, isLongString ? (_jsx("button", { type: "button", onClick: () => setStringExpanded((v) => !v), className: "text-[11px] font-medium text-blue-600 dark:text-blue-400 hover:underline px-1", children: stringExpanded ? 'Hide' : 'More' })) : null] })] }), isObjectLike && depth < MAX_STATE_DEPTH ? (_jsx(NestedStateTable, { value: value, depth: depth })) : null, stringExpanded && expandedString ? (_jsx("pre", { className: "mx-3 sm:mx-4 mb-2 max-h-40 overflow-auto rounded-lg bg-gray-50 dark:bg-gray-800/80 border border-gray-100 dark:border-gray-700 p-2 text-xs font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-words leading-relaxed", children: expandedString })) : null] }));
}
...
function MetaDataPanel({ smartObject, objectClass, mod, }) {
    ...
    if (objectClass) {
        rows.push({
            label: 'Class',
            short: 'constructor',
            value: (_jsx("span", { className: "font-medium text-gray-900 dark:text-white text-sm", children: objectClass })),
            copy: objectClass,
        });
    }
    if (mod) {
        rows.push({
            label: 'Module',
            short: 'mod',
            value: (_jsx(Link, { to: `/modules/${mod}`, className: "font-medium text-blue-600 dark:text-blue-400 hover:underline break-all font-mono text-xs", children: mod })),
            copy: mod,
        });
    }
    rows.push({
...
function Component({ title, compact }) {
    ...
    const [indexedIsObject, setIndexedIsObject] = useState(undefined);
    ...
    return (_jsxs("div", { className: "w-full space-y-5", children: [_jsxs("header", { children: [_jsx("p", { className: "text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-0.5", children: notSmartObject ? 'Output' : 'Smart object' }), _jsx("h1", { className: "mb-2 text-xl sm:text-2xl font-semibold dark:text-white", children: notSmartObject ? 'Not a smart object' : title || objectClass || 'Object' }), _jsxs("div", { className: "rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm", children: [_jsx("p", { className: "text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1", children: "Revision" }), _jsxs("div", { className: "flex flex-wrap items-center gap-1 font-mono text-xs sm:text-sm break-all", children: [_jsx(Link, { to: `/transactions/${txId}`, className: "font-medium text-blue-600 dark:text-blue-400 hover:underline", children: txId }), _jsxs("span", { className: "text-gray-700 dark:text-gray-300", children: [":", outNum] }), _jsx(Copy, { text: `${txId}:${outNum}` })] }), mod ? (_jsxs(_Fragment, { children: [_jsx("p", { className: "text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1 mt-3", children: "Module" }), _jsxs("div", { className: "flex flex-wrap items-center gap-1 font-mono text-xs sm:text-sm break-all", children: [_jsx(Link, { to: `/modules/${mod}`, className: "font-medium text-blue-600 dark:text-blue-400 hover:underline", children: mod }), _jsx(Copy, { text: mod })] })] })) : null, smartObject?._satoshis != null ? (_jsxs("p", { className: "mt-2 text-sm text-gray-600 dark:text-gray-400", children: [_jsx("span", { className: "font-medium text-gray-900 dark:text-white", children: smartObject._satoshis.toString() }), ' ', "sats", Array.isArray(smartObject._owners) && smartObject._owners.length > 0 ? (_jsxs(_Fragment, { children: [' · ', _jsxs("span", { className: "font-mono text-xs", children: [String(smartObject._owners[0]).slice(0, 12), "\u2026"] })] })) : null] })) : null] })] }), notSmartObject ? (_jsxs(InlineAlert, { variant: "info", title: "This output is not a smart object", children: [_jsx("p", { className: "mb-2", children: "It does not hold on-chain application state." }), _jsx("p", { className: "text-xs opacity-90", children: _jsx(Link, { to: `/transactions/${txId}`, className: "font-medium underline underline-offset-2 hover:opacity-100", children: "View transaction" }) })] })) : null, loadError && decryptDenied ? (_jsxs(InlineAlert, { variant: "info", title: "You cannot decrypt this object", children: [_jsx("p", { className: "mb-2", children: "Only wallets whose public key is listed in this object's _readers can read its state." }), _jsx("p", { className: "text-xs opacity-90", children: _jsx(Link, { to: `/transactions/${txId}`, className: "font-medium underline underline-offset-2 hover:opacity-100", children: "View transaction" }) })] })) : null, unknownLoadFailed ? (_jsxs(InlineAlert, { variant: "info", title: "Could not load this revision", children: [_jsx("p", { className: "mb-2", children: loadError }), _jsxs("p", { className: "text-xs opacity-90", children: ["It may not be a smart object, or it has not been indexed yet.", ' ', _jsx(Link, { to: `/transactions/${txId}`, className: "font-medium underline underline-offset-2 hover:opacity-100", children: "View transaction" })] })] })) : null, evalFailed ? (_jsxs(InlineAlert, { variant: "error", title: "Could not load object", onDismiss: () => setLoadError(null), children: [_jsx("p", { className: "mb-2", children: loadError }), _jsxs("p", { className: "text-xs opacity-90", children: ["The node failed to evaluate this smart object.", ' ', _jsx(Link, { to: `/transactions/${txId}`, className: "font-medium underline underline-offset-2 hover:opacity-100", children: "View transaction" })] })] })) : null, loading ? (_jsxs("div", { className: "animate-pulse space-y-4", children: [_jsx("div", { className: "h-36 rounded-xl bg-gray-200 dark:bg-gray-700" }), _jsx("div", { className: "h-48 rounded-xl bg-gray-200 dark:bg-gray-700" }), _jsx("div", { className: "h-40 rounded-xl bg-gray-200 dark:bg-gray-700" }), _jsx("div", { className: "h-28 rounded-xl bg-gray-200 dark:bg-gray-700" })] })) : null, smartObject ? (_jsxs(_Fragment, { children: [_jsxs("section", { className: "rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm overflow-hidden", "aria-label": "State", children: [_jsxs("div", { className: "px-4 py-3 border-b border-gray-200 dark:border-gray-700", children: [_jsx("h2", { className: "text-base sm:text-lg font-semibold text-gray-900 dark:text-white", children: "State" }), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-400 mt-0.5", children: "Public properties of this smart object at the current revision \u2014 the data stored on chain after evaluation (not including system fields like _id or _owners)." })] }), _jsx(SmartObjectValues, { smartObject: smartObject })] }), _jsx(SmartObjectFunctions, { smartObject: smartObject, functionsExist: functionsExist, options: options, latestRev: latest, compact: compact })] })) : null, !notSmartObject ? (_jsx(RevisionHistory, { prev: prev, next: next, first: first, latest: latest, current: rev, chain: timeline, loading: timelineLoading, ancestorTxIds: ancestorTxIds })) : null, smartObject ? (_jsx(MetaDataPanel, { smartObject: smartObject, objectClass: objectClass, mod: mod })) : null] }));
}
