...
function classNameFromExports(smartObject, exports) {
  ...
  for (const name of Object.getOwnPropertyNames(exports)) {
    ...
    try {
      if (proto && proto === value.prototype) return name
      if (smartObject instanceof value) return name
    } catch {
      // continue
    }
  }
...
function Copy({ text }) {
  ...
  return _jsx('button', {
    ...
    className:
      'inline-flex items-center cursor-pointer pl-1.5 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white focus:outline-none shrink-0',
    'aria-label': 'Copy',
    children: copied
      ? _jsx(HiCheck, { className: 'w-4 h-4 text-green-500' })
      : _jsx(HiOutlineClipboard, { className: 'w-4 h-4' }),
  })
}
...
function TypeBadge({ type }) {
  return _jsx('span', {
    className:
      'inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400',
    children: type,
  })
}
function formatStatePreview(value) {
  if (value === null) {
    return {
      type: 'null',
      preview: _jsx('span', { className: 'text-gray-400 italic', children: 'null' }),
      full: 'null',
    }
  }
  if (value === undefined) {
  ...
  if (typeof value === 'number' || typeof value === 'bigint') {
    return {
      type: typeof value,
      preview: _jsx('span', {
        className: 'tabular-nums font-medium text-gray-900 dark:text-white',
  ...
  if (typeof value === 'string') {
    ...
    return {
      ...
      preview: _jsx('span', {
        ...
        children:
          value === ''
            ? _jsx('span', { className: 'text-gray-400 italic', children: 'empty' })
            : short,
      }),
      full: value,
    }
  ...
  if (Array.isArray(value)) {
    ...
    return {
      ...
      preview: _jsxs('span', {
        ...
        children: [value.length, ' item', value.length === 1 ? '' : 's'],
      }),
      full,
    }
  }
...
function NestedStateTable({ value, depth }) {
  ...
  return _jsxs('div', {
    className:
      'mx-3 sm:mx-4 mb-2 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50/70 dark:bg-gray-800/40 overflow-hidden',
    children: [
      nestedRev
        ? _jsx('div', {
            className: 'px-3 py-1.5 border-b border-gray-100 dark:border-gray-700',
            children: _jsxs(Link, {
              to: `/objects/${nestedRev}`,
              className:
                'text-[11px] font-medium text-blue-600 dark:text-blue-400 hover:underline font-mono break-all',
              children: ['View object ', truncateRev(nestedRev), ' \u2192'],
            }),
          })
        : null,
      entries.length === 0
        ? _jsx('p', {
            ...
            children: 'No public properties.',
          })
        : entries.map(([key, nested]) =>
            _jsx(StateValueRow, { name: key, value: nested, nested: true, depth: depth + 1 }, key),
          ),
    ],
  })
}
function StateValueRow({ name, value, nested = false, depth = 0 }) {
  ...
  return _jsxs('div', {
    ...
    children: [
      _jsxs('div', {
        ...
        children: [
          _jsxs('div', {
            className: 'shrink-0 flex items-center gap-1.5 w-[7.5rem] sm:w-36',
            children: [
              _jsx('span', {
                className:
                  'text-xs font-medium text-gray-800 dark:text-gray-200 whitespace-nowrap truncate',
                children: capitalizeFirstLetter(name),
              }),
              _jsx('code', {
                className:
                  'text-[10px] text-gray-400 dark:text-gray-500 whitespace-nowrap hidden sm:inline',
                children: name,
              }),
            ],
          }),
          _jsx(TypeBadge, { type: type }),
          _jsx('div', {
          ...
          _jsxs('div', {
            ...
            children: [
              full && full !== 'null' && full !== 'undefined' ? _jsx(Copy, { text: full }) : null,
              isLongString
                ? _jsx('button', {
                    type: 'button',
                    onClick: () => setStringExpanded((v) => !v),
                    className:
                      'text-[11px] font-medium text-blue-600 dark:text-blue-400 hover:underline px-1',
                    children: stringExpanded ? 'Hide' : 'More',
                  })
                : null,
            ],
          }),
      ...
      isObjectLike && depth < MAX_STATE_DEPTH
        ? _jsx(NestedStateTable, { value: value, depth: depth })
        : null,
      stringExpanded && expandedString
        ? _jsx('pre', {
            className:
              'mx-3 sm:mx-4 mb-2 max-h-40 overflow-auto rounded-lg bg-gray-50 dark:bg-gray-800/80 border border-gray-100 dark:border-gray-700 p-2 text-xs font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-words leading-relaxed',
            children: expandedString,
          })
        : null,
    ],
  })
...
const SmartObjectValues = ({ smartObject }) => {
  ...
  if (entries.length === 0) {
    return _jsx('p', {
      ...
      children: 'No public properties on this object.',
    })
  }
  return _jsx('div', {
    children: entries.map(([key, value]) => _jsx(StateValueRow, { name: key, value: value }, key)),
  })
}
/**
...
function RevisionHistory({ prev, next, first, latest, current, chain, loading, ancestorTxIds }) {
  ...
  return _jsxs('section', {
    ...
    children: [
      _jsxs('div', {
        className:
          'px-4 py-3 flex flex-wrap items-center justify-between gap-2 border-b border-gray-200 dark:border-gray-700',
        children: [
          _jsxs('div', {
            children: [
              _jsx('h2', {
          ...
          _jsxs('div', {
            ...
            children: [
              isFirst
                ? _jsx('span', {
                    className:
                      'inline-flex items-center rounded-full px-2 py-0.5 font-medium bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200',
                    children: 'First',
                  })
                : null,
              isLatest
                ? _jsx('span', {
                    className:
                      'inline-flex items-center rounded-full px-2 py-0.5 font-medium bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300',
                    children: 'Latest',
                  })
                : _jsx('span', {
                    className:
                      'inline-flex items-center rounded-full px-2 py-0.5 font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300',
                    children: 'Historical',
                  }),
            ],
          }),
      ...
      _jsxs('div', {
        ...
        children: [
          _jsxs('div', {
            ...
            children: [
              first && first !== current
                ...
                : _jsx('span', { className: `${btnBase} ${btnDisabled}`, children: 'First' }),
              prev
                ? _jsxs(Link, {
                    to: `/objects/${prev}`,
              ...
              latest && latest !== current
                ...
                : _jsx('span', { className: `${btnBase} ${btnDisabled}`, children: 'Latest' }),
            ],
          }),
          loading
            ? _jsxs('div', {
                className: 'animate-pulse space-y-2 ms-2',
                children: [
                  _jsx('div', { className: 'h-14 bg-gray-200 dark:bg-gray-700 rounded-lg w-full' }),
                  _jsx('div', { className: 'h-14 bg-gray-200 dark:bg-gray-700 rounded-lg w-5/6' }),
                  _jsx('div', { className: 'h-14 bg-gray-200 dark:bg-gray-700 rounded-lg w-4/6' }),
                ],
              })
            : chain.length === 0
              ? _jsx('p', {
                  className: 'text-sm text-gray-500 dark:text-gray-400',
                  children: 'No revision history available.',
                })
              : _jsx('ol', {
                  className: 'relative border-s border-gray-200 dark:border-gray-700 ms-2',
                  children: chain.map((r, i) => {
                          ...
                          _jsxs('div', {
                            ...
                            children: [
                              _jsxs('div', {
                                ...
                                children: [
                                  ...
                                  isChainFirst
                                    ? _jsx('span', {
                                        className:
                                          'text-[10px] rounded px-1.5 py-0.5 bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200',
                                        children: 'first',
                                      })
                                    : null,
                                  isChainLast
                                    ? _jsx('span', {
                                        className:
                                          'text-[10px] rounded px-1.5 py-0.5 bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300',
                                        children: 'latest',
                                      })
                                    : null,
                                  isCurrent
                                    ? _jsx('span', {
                                        className:
                                          'text-[10px] rounded px-1.5 py-0.5 bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200',
                                        children: 'viewing',
                                      })
                                    : null,
                                ],
                              ...
                              isCurrent
                                ? _jsx('p', {
                                    className:
                                      'font-mono text-xs text-gray-900 dark:text-white break-all',
                                    children: r,
                                  })
                                : _jsx(Link, {
                                    to: `/objects/${r}`,
                                    className:
                                      'font-mono text-xs text-blue-600 dark:text-blue-400 hover:underline break-all',
                                    children: r,
                                  }),
                              _jsx('div', {
                                className: 'mt-1',
                                children: _jsx(Link, {
                                  ...
                                  className:
                                    'text-[11px] text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400',
          ...
          (prev || next) &&
            _jsxs('div', {
              className:
                'grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-xs font-mono text-gray-600 dark:text-gray-400 pt-1 border-t border-gray-100 dark:border-gray-800',
              children: [
                prev
                  ? _jsxs('p', {
                      className: 'truncate',
                      ...
                      children: [
                        _jsx('span', {
                          className: 'text-gray-400 dark:text-gray-500 font-sans',
                          children: 'prev ',
                        }),
                        _jsx(Link, {
                          to: `/objects/${prev}`,
                          className: 'text-blue-600 dark:text-blue-400 hover:underline',
                          children: truncateRev(prev),
                        }),
                      ],
                    })
                  : _jsx('p', {
                      className: 'text-gray-400 font-sans',
                      children: 'No previous revision',
                    }),
                next
                  ? _jsxs('p', {
                      ...
                      children: [
                        ...
                        _jsx(Link, {
                          to: `/objects/${next}`,
                          className: 'text-blue-600 dark:text-blue-400 hover:underline',
                          children: truncateRev(next),
                        }),
                      ],
                    })
                  : _jsx('p', {
                      className: 'text-gray-400 sm:text-right font-sans',
                      children: 'No next revision',
                    }),
              ],
            }),
          ancestorTxIds.length > 0
            ? _jsxs('details', {
                ...
                children: [
                  _jsxs('summary', {
                    className:
                      'cursor-pointer text-xs font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200',
                    children: ['Ancestor transactions (', ancestorTxIds.length, ')'],
                  }),
                  _jsx('ul', {
                    className:
                      'mt-2 space-y-1 max-h-32 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40 p-2',
                    children: ancestorTxIds.map((txId) =>
                      _jsx(
                        'li',
                        {
                          children: _jsx(Link, {
...
function MetaDataPanel({ smartObject, objectClass, mod }) {
  if (!smartObject) return null
  const rows = []
  if (objectClass) {
    rows.push({
  ...
  if (mod) {
    rows.push({
      ...
      value: _jsx(Link, {
        to: `/modules/${mod}`,
        className:
          'font-medium text-blue-600 dark:text-blue-400 hover:underline break-all font-mono text-xs',
        children: mod,
      }),
      copy: mod,
    })
  ...
  rows.push(
    ...
    },
    {
      ...
      value: _jsx('span', {
        ...
        children: Array.isArray(smartObject._owners)
    ...
    },
    {
      ...
      short: '_satoshis',
      value: _jsxs('span', {
        className: 'font-medium text-gray-900 dark:text-white tabular-nums text-sm',
        children: [
          smartObject._satoshis?.toString?.() ?? String(smartObject._satoshis ?? '—'),
          _jsx('span', {
            className: 'ml-1 text-xs font-normal text-gray-500 dark:text-gray-400',
            children: 'sats',
          }),
        ],
      }),
    ...
    children: [
      _jsxs('div', {
        ...
        children: [
          ...
          _jsx('p', {
            className: 'text-xs text-gray-500 dark:text-gray-400 mt-0.5',
            children: 'On-chain identity fields for this smart object',
          }),
        ],
      }),
      _jsx('dl', {
        ...
        children: rows.map((row) =>
          _jsxs(
            'div',
            {
              ...
              children: [
                _jsxs('dt', {
                  className: 'shrink-0 flex items-center gap-1.5 w-[9.5rem] sm:w-40',
                  children: [
                    _jsx('span', {
                      className:
                        'text-xs font-medium text-gray-800 dark:text-gray-200 whitespace-nowrap',
                      children: row.label,
                    }),
                    _jsx('code', {
                ...
                _jsxs('dd', {
                  className:
                    'min-w-0 flex-1 flex items-center gap-0.5 text-xs text-gray-700 dark:text-gray-300',
                  children: [
                    _jsx('span', {
                      className: 'min-w-0 flex-1 truncate sm:whitespace-normal sm:break-all',
                      children: row.value,
                    }),
                    row.copy ? _jsx(Copy, { text: row.copy }) : null,
                  ],
                }),
              ],
            },
...
function Component({ title }) {
  ...
  useEffect(() => {
    ...
    const fetchMod = async () => {
      try {
        const rows = await computer.getOTXOs({ rev, verbosity: 1 })
        if (cancelled) return
        const m = rows[0]?.mod
    ...
    children: [
      _jsxs('header', {
        children: [
          ...
          _jsx('h1', {
            className: 'mb-2 text-xl sm:text-2xl font-semibold dark:text-white',
            children: title || objectClass || 'Object',
          }),
          _jsxs('div', {
            className:
            ...
            children: [
              _jsx('p', {
                className:
                  'text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1',
                children: 'Revision',
              }),
              _jsxs('div', {
                className:
                  'flex flex-wrap items-center gap-1 font-mono text-xs sm:text-sm break-all',
                children: [
                  _jsx(Link, {
                    to: `/transactions/${txId}`,
                    className: 'font-medium text-blue-600 dark:text-blue-400 hover:underline',
              ...
              mod
                ? _jsxs(_Fragment, {
                    children: [
                      _jsx('p', {
                        className:
                          'text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1 mt-3',
                        children: 'Module',
                      }),
                      _jsxs('div', {
                        className:
                ...
                ? _jsxs('p', {
                    ...
                    children: [
                      _jsx('span', {
                        ...
                        children: smartObject._satoshis.toString(),
                      }),
                      ' ',
                      'sats',
                        ...
                        ? _jsxs(_Fragment, {
                            children: [
                              ...
                              _jsxs('span', {
                                ...
                                children: [String(smartObject._owners[0]).slice(0, 12), '\u2026'],
                              }),
                            ],
                          })
                        : null,
      ...
      loadError && decryptDenied
        ? _jsxs(InlineAlert, {
            variant: 'info',
            title: 'You cannot decrypt this object',
            children: [
              _jsx('p', {
                className: 'mb-2',
                children:
                  "Only wallets whose public key is listed in this object's _readers can read its state.",
              }),
              _jsx('p', {
      ...
      loadError && !decryptDenied
        ? _jsxs(InlineAlert, {
            ...
            children: [
              _jsx('p', { className: 'mb-2', children: loadError }),
              _jsxs('p', {
                className: 'text-xs opacity-90',
                children: [
      ...
      loading
        ? _jsxs('div', {
            ...
            children: [
              _jsx('div', { className: 'h-36 rounded-xl bg-gray-200 dark:bg-gray-700' }),
              _jsx('div', { className: 'h-48 rounded-xl bg-gray-200 dark:bg-gray-700' }),
              _jsx('div', { className: 'h-40 rounded-xl bg-gray-200 dark:bg-gray-700' }),
              _jsx('div', { className: 'h-28 rounded-xl bg-gray-200 dark:bg-gray-700' }),
            ],
          })
        : null,
      smartObject
                  ...
                  _jsxs('div', {
                    ...
                    children: [
                      ...
                      _jsx('p', {
                        className: 'text-xs text-gray-500 dark:text-gray-400 mt-0.5',
                        children:
                          'Public properties of this smart object at the current revision \u2014 the data stored on chain after evaluation (not including system fields like _id or _owners).',
                      }),
                    ],
                  }),
                  _jsx(SmartObjectValues, { smartObject: smartObject }),
                ],
              }),
              _jsx(SmartObjectFunctions, {
      ...
      smartObject
        ? _jsx(MetaDataPanel, { smartObject: smartObject, objectClass: objectClass, mod: mod })
        : null,
    ],
  })
}
