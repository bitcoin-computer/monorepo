...
function FromRecords({ records, computer, }) {
    return (_jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4 w-full", children: records.map((record) => (_jsx("div", { className: "min-w-0", children: _jsx(Link, { to: `/objects/${record.rev}`, className: "block h-full focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-lg", children: _jsx(ObjectCard, { record: record, computer: computer }) }) }, record.rev))) }));
}
...
function EmptyObjectsState({ hasFilters, embed, }) {
    if (embed) {
        return (_jsx("div", { className: "w-full py-8 px-4 text-center rounded-lg border border-dashed border-gray-300 dark:border-gray-600", children: _jsx("p", { className: "text-sm text-gray-600 dark:text-gray-400", children: hasFilters ? 'No objects match this filter' : 'No smart objects yet' }) }));
    }
...
export function GalleryWithPagination(q = {}) {
    ...
    return (_jsxs("div", { className: "relative w-full", children: [embed ? null : (_jsxs("header", { className: "mb-4 flex flex-wrap items-center justify-between gap-3", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-xl sm:text-2xl font-semibold dark:text-white", children: "Smart objects" }), _jsxs("p", { className: "text-sm text-gray-500 dark:text-gray-400", children: ["Unspent on-chain application state", records.length > 0
                                        ? ` · ${records.length}${isNextAvailable ? '+' : ''} on this page`
                                        : ''] })] }), _jsx(Link, { to: "/playground", className: "inline-flex items-center px-3 py-1.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-500", children: "Create" })] })), embed ? null : (_jsx(ActiveFilters, { publicKey: fromUrl.publicKey || q.publicKey, mod: fromUrl.mod || q.mod, address: fromUrl.address || q.address, order: fromUrl.order || q.order })), listLoading && records.length === 0 ? _jsx(GallerySkeletons, {}) : null, listError && !listLoading ? (_jsx("div", { className: "mb-3", children: _jsx(InlineAlert, { variant: "error", children: listError }) })) : null, !listLoading && showNoAsset ? (_jsx(EmptyObjectsState, { hasFilters: hasFilters, embed: embed })) : null, records.length > 0 ? _jsx(FromRecords, { records: records, computer: computer }) : null, !(pageNum === 0 && records.length === 0) && !listLoading ? (_jsx(Pagination, { isPrevAvailable: isPrevAvailable, handlePrev: handlePrev, isNextAvailable: isNextAvailable, handleNext: handleNext })) : null] }));
}
