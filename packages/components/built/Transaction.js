...
function SpendsCell({ utxo }) {
    ...
    return (_jsx(Link, { to: `/transactions/${spTxid}`, className: "font-medium text-blue-600 dark:text-blue-400 hover:underline font-mono text-xs", title: spends, children: trimmed }));
}
function SmartObjectBadge({ isObject }) {
    if (isObject === true) {
        return (_jsx("span", { className: "inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300", children: "Yes" }));
    }
    if (isObject === false) {
        return (_jsx("span", { className: "inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300", children: "No" }));
    }
    return _jsx("span", { className: "text-gray-400 dark:text-gray-500", children: "\u2014" });
}
function OutputIndexCell({ txn, n, isObject, }) {
    if (txn && isObject === true) {
        return (_jsx(Link, { to: `/objects/${txn}:${n}`, className: "font-medium text-blue-600 dark:text-blue-400 hover:underline tabular-nums", children: n }));
    }
    return _jsx("span", { className: "tabular-nums text-gray-900 dark:text-white", children: n });
}
export function OutputsComponent({ rpcTxnData, txn, }) {
    const computer = useContext(ComputerContext);
    const [flags, setFlags] = useState({});
    const vouts = Array.isArray(rpcTxnData?.vout) ? rpcTxnData.vout : [];
    useEffect(() => {
        const outs = Array.isArray(rpcTxnData?.vout) ? rpcTxnData.vout : [];
        ...
        setFlags({});
        const load = async () => {
            const entries = await Promise.all(outs.map(async (output) => {
                const flag = await lookupIsObject(computer, `${txn}:${output.n}`);
                return [output.n, flag];
            }));
            if (!cancelled)
    ...
    return (_jsxs("section", { className: "w-full", children: [_jsx("h2", { className: "mb-2 text-base sm:text-lg font-semibold dark:text-white", children: "Outputs" }), _jsx("div", { className: "relative overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm", children: _jsxs("table", { className: "w-full text-sm text-left text-gray-500 dark:text-gray-400", children: [_jsx("thead", { className: "text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700/80 dark:text-gray-300", children: _jsxs("tr", { children: [_jsx("th", { scope: "col", className: "px-4 py-3", children: "#" }), _jsx("th", { scope: "col", className: "px-4 py-3", children: "Value" }), _jsx("th", { scope: "col", className: "px-4 py-3", children: "Type" }), _jsx("th", { scope: "col", className: "px-4 py-3", children: "Smart object" }), _jsx("th", { scope: "col", className: "px-4 py-3 hidden md:table-cell", children: "Script" }), _jsx("th", { scope: "col", className: "px-4 py-3", children: "Spent by" })] }) }), _jsx("tbody", { children: vouts.map((output) => (_jsxs("tr", { className: "bg-white border-b last:border-0 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/40", children: [_jsx("td", { className: "px-4 py-3", children: _jsx(OutputIndexCell, { txn: txn, n: output.n, isObject: flags[output.n] }) }), _jsx("td", { className: "px-4 py-3 tabular-nums font-medium text-gray-900 dark:text-white", children: formatRpcValue(output.value) }), _jsx("td", { className: "px-4 py-3", children: _jsx("span", { className: "inline-flex rounded px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200", children: output.scriptPubKey?.type || '—' }) }), _jsx("td", { className: "px-4 py-3", children: _jsx(SmartObjectBadge, { isObject: flags[output.n] }) }), _jsx("td", { className: "px-4 py-3 break-all font-mono text-xs max-w-xs truncate hidden md:table-cell", title: output.scriptPubKey?.asm, children: output.scriptPubKey?.asm || '—' }), _jsx("td", { className: "px-4 py-3", children: txn ? _jsx(SpendsCell, { utxo: `${txn}:${output.n}` }) : _jsx("span", { children: "\u2014" }) })] }, output.n))) })] }) })] }));
}
export const outputsComponent = OutputsComponent;
function InputRevCell({ utxo, checkForSpentInput }) {
    const computer = useContext(ComputerContext);
    const [spends, setSpends] = useState(null);
    const [isObject, setIsObject] = useState(undefined);
    useEffect(() => {
        let cancelled = false;
...
export function TransactionComponent() {
    ...
    return (_jsxs("div", { className: "w-full space-y-4", children: [_jsxs("header", { children: [_jsx("p", { className: "text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-0.5", children: "Transaction" }), _jsx("h1", { className: "text-xl sm:text-2xl font-semibold dark:text-white", children: "Details" })] }), loading ? (_jsxs("div", { className: "space-y-4 animate-pulse", children: [_jsx("div", { className: "grid grid-cols-2 sm:grid-cols-4 gap-3", children: [1, 2, 3, 4].map((i) => (_jsx("div", { className: "h-20 rounded-xl bg-gray-200 dark:bg-gray-700" }, i))) }), _jsx("div", { className: "h-40 rounded-xl bg-gray-200 dark:bg-gray-700" })] })) : null, error && !loading ? (_jsxs("div", { className: "rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/40 p-5", children: [_jsx("p", { className: "font-semibold text-red-800 dark:text-red-300 mb-1", children: "Transaction not found" }), _jsx("p", { className: "text-sm text-red-700 dark:text-red-400 mb-3", children: error }), _jsx("p", { className: "text-xs font-mono break-all text-red-600/80 dark:text-red-400/80", children: txn }), _jsxs("p", { className: "mt-3 text-sm text-gray-600 dark:text-gray-400", children: ["Tip: 64-character hex values are treated as transaction ids. To filter objects by owner public key, use a compressed key (66 hex, starting with 02/03) or open", ' ', _jsx("code", { className: "text-xs bg-gray-100 dark:bg-gray-800 px-1 rounded", children: "/?publicKey=\u2026" }), "."] })] })) : null, !loading && !error && rpcTxnData ? _jsx(TxSummary, { rpcTxnData: rpcTxnData, txn: txn }) : null, !loading && !error && !rpcTxnData && txnData ? (_jsxs("div", { className: "rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 mb-4 shadow-sm", children: [_jsxs("div", { className: "flex items-start gap-2", children: [_jsx("p", { className: "font-mono text-xs sm:text-sm break-all flex-1 dark:text-white", children: txn }), _jsx(CopyIconButton, { text: txn })] }), _jsx("p", { className: "mt-2 text-sm text-amber-700 dark:text-amber-400", children: "Loaded raw transaction; RPC details unavailable." })] })) : null, !loading && !error && transition ? transitionComponent({ transition }) : null, !loading && !error && !transition && decodeFailure ? (_jsx(TransitionUnavailable, { kind: decodeFailure, error: decodeError ?? undefined, txn: txn })) : null, !loading && !error && rpcTxnData?.vin
                ...
                : null, !loading && !error && rpcTxnData?.vout ? (_jsx(OutputsComponent, { rpcTxnData: rpcTxnData, txn: txn })) : null] }));
}
