...
import { getEnv, bigIntToStr, strToBigInt, validChain, validNetwork, validPath, validUrl, } from './common/utils';
...
function WalletContent({ computer, modSpecs, isOpen, }) {
    ...
    return (_jsxs("div", { className: "pb-4", children: [_jsx(BalanceCard, { computer: computer, balance: balance, isRefreshing: isRefreshing, onRefresh: refreshBalance, onFund: fund }), _jsx(DepositAddress, { computer: computer }), _jsx("hr", { className: "h-px my-4 bg-gray-200 border-0 dark:bg-gray-700" }), _jsx(SendForm, { computer: computer, modSpecs: modSpecs, balance: balance, onSuccess: refreshBalance }), VITE_WITHDRAW_MOD_SPEC ? (_jsx(Collapsible, { title: "Advanced withdraw (app payments)", children: _jsx(LegacyWithdrawAll, { computer: computer, payments: payments, onSuccess: refreshBalance }) })) : null, _jsx("hr", { className: "h-px my-4 bg-gray-200 border-0 dark:bg-gray-700" }), _jsxs(Collapsible, { title: "Account & security", children: [_jsx(PublicKeyField, { computer: computer }), _jsx(MnemonicField, { computer: computer }), !validChain(getEnv('CHAIN')) ? (_jsx(SimpleInfo, { label: "Chain", value: computer.getChain() })) : null, !validNetwork(getEnv('NETWORK')) ? (_jsx(SimpleInfo, { label: "Network", value: computer.getNetwork() })) : null, !validUrl(getEnv('URL')) ? (_jsx(SimpleInfo, { label: "Node URL", value: computer.getUrl() })) : null, !validPath(getEnv('PATH')) ? (_jsx(SimpleInfo, { label: "Derivation path", value: computer.getPath() })) : null] }), _jsx(Collapsible, { title: "Log out", tone: "danger", children: _jsx(LogOutSection, {}) })] }));
}
